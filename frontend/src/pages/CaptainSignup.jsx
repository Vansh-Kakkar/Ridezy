...
const CaptainSignup = () => {
  ...
  const submitHandler = async (e) => {
    ...
    const updatedCaptain = { fullname, email, password, vehicle };
    setCaptainData(updatedCaptain);

    const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/captains/register`, updatedCaptain);
    ...
    <div className="py-10 px-8 max-w-2xl mx-auto h-full flex flex-col justify-between">
      <div>
        ...
        <form onSubmit={submitHandler} className="space-y-6">
          ...
          <button className="bg-black text-white font-semibold rounded px-4 py-2 w-full text-lg">
      ...
      <footer className="mt-10 text-sm text-center text-gray-600">
        By proceeding you consent to get mails, including by automated means, from Ridezy to the email provided.
      </footer>
    </div>
  );
};
